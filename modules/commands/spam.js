...
async function onCall({ message, args }) {
	...
	if (isStop) {
		...
		if (index > -1) {
			...
			return message.reply("ÄÃ£ dá»«ng spam");
		} else {
			return message.reply("ChÆ°a báº¯t Ä‘áº§u spam");
		}
	} else {
		...
		if (global.spam.indexOf(message.threadID) > -1) {
			...
			while (global.spam.indexOf(message.threadID) > -1) {
				...
				await new Promise(resolve => setTimeout(resolve, 5000)); // delay 3000 mili giÃ¢y = 3 giÃ¢y
			}
		}
	}
}
